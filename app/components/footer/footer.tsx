...
const socialItems = ["facebook", "instagram", "whatsapp"] as const;
...
export default function Footer({ locale }: FooterProps) {
  ...
  if (isContactPage) {
    return (
      <footer data-nosnippet className="mt-12 border-t border-black/10 bg-white">
        {mapAndPatent}
      </footer>
    );
  }
      ...
      <div className="mx-auto flex min-h-[320px] max-w-7xl flex-col gap-8 px-6 py-12 sm:flex-row sm:items-center sm:justify-around">
        ...
        <div className="flex flex-col gap-3">
          ...
          {socialItems.map((item) => (
            <div key={item} className="flex items-center gap-2 text-sm text-black/55">
              <span>{t(item)}</span>
              <svg
                viewBox="0 0 24 24"
                aria-hidden="true"
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link
            ...
            className="text-sm text-black/55 transition-colors hover:text-black"
          >
            {menuLabels[locale].home}
          </Link>

          <Link
            href={withLocale("/service")}
            className="text-sm text-black/55 transition-colors hover:text-black"
          >
            {menuLabels[locale].service}
          </Link>

          <Link
            href={withLocale("/reserve")}
            className="text-sm text-black/55 transition-colors hover:text-black"
          >
            {menuLabels[locale].reserve}
          </Link>
        </div>
      </div>
