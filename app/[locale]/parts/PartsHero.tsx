...
const categories = ["Wheels & Tires", "Body Parts", "Interior", "Maintenance", "Charging"] as const;

type PartsHeroProps = {
  locale: "en" | "ka" | "ru";
};

export function PartsHero({ locale }: PartsHeroProps) {
  useTeslaReveal();

  const categoryLinks = categories.map((cat, idx) => ({
    label: cat,
    href: `/${locale}/parts#parts-catalog`,
    index: idx,
  }));

  return (
    <section className="relative hidden h-[90vh] w-full overflow-hidden bg-black text-white md:block">
      <div className="relative z-10 flex h-full flex-col items-center justify-center px-6 text-center">
        ...
        <div className="reveal-hidden reveal-scale-in stagger-2 mt-12 flex flex-col items-center gap-4 sm:flex-row">
          ...
          >
            <Link href={`/${locale}/parts#parts-catalog`} className="flex items-center justify-center gap-2">
          ...
          >
            <Link href={`/${locale}/service`} className="text-sm font-bold uppercase tracking-wide">Find Service</Link>
          </Button>
        </div>
        ...
        <div className="reveal-hidden reveal-fade-right stagger-3 absolute bottom-6 hidden w-full max-w-5xl justify-between border-t border-white/10 pt-9 lg:flex">
          {categoryLinks.map(({ label, href, index }) => (
            ...
            >
              <span className="text-[10px] font-black uppercase tracking-widest text-red-500">0{index + 1}</span>
